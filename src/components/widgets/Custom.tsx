...
import { ThemeConfig } from '~/lib/themes/theme-config.type';
import { ThemeModes } from '~/lib/themes/theme-modes';
import { ThemeBaseColors } from '~/lib/themes/theme-base-colors';
import { ThemeBorderRadiuses } from '~/lib/themes/theme-border-radiuses';
...
export default component$<PropsOf<typeof Button>>(() => {
  ...
  return (
        ...
        <div class="flex justify-end">
          <div class="grid grid-cols-[repeat(22,0fr)]">
            {Object.values(ThemePrimaryColors).map((primaryColor: string) => {
              ...
              if (primaryColor.includes('slate-100') ||
        ...
        <div>
          ...
          <div class="flex h-12 space-x-3">
            {Object.values(ThemeBorderRadiuses).map((borderRadius: string) => {
              const isActive = themeComputedObjectSig.value.borderRadius === borderRadius;
              return (
                <Button
                  key={borderRadius}
                  look="outline"
        ...
        <Button
          look="ghost"
          onClick$={() => {
            const resetMode = themeComputedObjectSig.value.mode === 'dark' ? 'dark' : 'light';
